...
| 会議室空き状況カレンダー | SharePointカレンダー（手動入力ベース） | 開発者さんが初期構築 |
...
|------|------|
| Form bridge（問い合わせフォーム） | Microsoft Forms（SharePointページ埋め込み） |
| レポトン（PDF見積書生成） | Power Automate + Wordテンプレート |
| K viewer カレンダー（会議室空き状況） | SharePointカレンダー |
...
- 会議室空き状況カレンダー（SharePoint、手動入力ベース）
- 見積メール送信の日帰り/宿泊分岐（日帰り:自動送信、宿泊:下書き作成→担当者が空き確認後に手動送信）
